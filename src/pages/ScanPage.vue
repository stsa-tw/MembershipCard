  ...
  <div v-else class="text-center q-pa-sm">
    <q-btn
      @click="
        ...
        valid = false;
      "
      :label="$t('scanPage.scanAgain')"
      icon="refresh"
      color="primary"
      class="text-h4 full-width"
    />
    <q-icon v-if="valid" name="check_circle" color="green" size="100px" class="q-mt-lg" />
    <q-icon v-else name="cancel" color="red" size="100px" class="q-mt-lg" />
    <div v-if="valid" class="text-h4 q-mt-lg">{{ $t('scanPage.validCode') }}</div>
    <div v-else class="text-h4 q-mt-lg">{{ $t('scanPage.invalidCode') }}</div>
    <div v-if="error" class="text-h4 q-mt-md text-red-500">{{ error }}</div>
    <div v-if="valid" class="text-h5 q-mt-md">{{ $t('scanPage.name') }}:</div>
    <div v-if="valid" class="text-h6 q-mt-sm">{{ token.name }}</div>
    <div v-if="valid" class="text-h5 q-mt-md">{{ $t('scanPage.username') }}:</div>
    <div v-if="valid" class="text-h6 q-mt-sm">{{ token.username }}</div>
    <div v-if="valid" class="text-h5 q-mt-md">{{ $t('scanPage.email') }}:</div>
